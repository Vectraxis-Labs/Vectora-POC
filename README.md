...
- **An AI research workspace** — a version-controlled workspace with an AI agent that guides your research from start to finish.
...
| Semantic search | Full Gitea version control |
| Vectora AI (Q&A, summarize, translate, TTS) | Voice & video calls |
| Research workspace with notes & versioning | In-app PDF viewer |
...
| LLM API (light demo usage) | $10–30 |
| Domain + DNS | $1–2 |
...
The PoC is built across 6 phases, each building on the last:
...
| Phase 4 | Research workspace, AI agent, task tracker | 14–18 days |
| Phase 5 | Peer connections, discovery, real-time chat | 7–10 days |
| Phase 6 | Teams, calendar, notifications, polish | 10–14 days |

**Total estimated: 60–82 days for a solo developer working full-time.**
...
4. **Build the full file system** with Gitea + DVC + S3-backed dataset storage
5. **Implement** the 10-stage data quality pipeline
6. **Deploy multi-region** — Mumbai (ap-south-1) + N. Virginia (us-east-1), Active-Active
7. **Add** in-app PDF viewer, voice/video calls, payments, and email notifications
...
**Vectora AI** — The AI agent embedded throughout the platform. At the paper level, it can answer questions, summarize, translate, and read papers aloud. Inside a research workspace, it acts as a project guide — helping plan the research, write code, create documents, and suggest next steps.

**Research Workspace** — A private, version-controlled space for a research project. Contains research papers, a file system (managed by you and the AI), and a Jira-like task tracker. Peers can be added for collaboration.

**Peer Network** — A mutual connection system (like LinkedIn). Peers can text, call, and video call each other, and be added to research projects and teams.
